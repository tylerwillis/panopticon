...
def test_multiple_workflows_are_configurable_and_run_concurrently(tmp_path: Path) -> None:
    ...
    with TestClient(app) as client:
        ...
        assert {"spike", "github-peer-reviewed", "custom"} <= {
            w["name"] for w in client.get("/workflows").json()
        }
        ...
        assert (
            states["spike"] == "ITERATING" and states["custom"] == "ONLY"
        )  # each its workflow's start
        assert states["github-peer-reviewed"] not in (
            "ITERATING",
            "ONLY",
        ...
        gpr_skills, spike_skills = (
            _skill_names(client, tasks["github-peer-reviewed"]),
            _skill_names(client, tasks["spike"]),
        )
