# REQ-014: Auto-advance on responsibilities met
...
4. `resolve_responsibility` MUST NOT transition a task out of a state that has no
   ...
   declared `advance`).
5. In the case of REQ-014.1.4, the task MUST remain in that state with every
   responsibility resolved.

### REQ-014.2: Parity with an explicit advance

1. A transition performed under REQ-014.1.1 MUST append the same history entry,
...
2. The task object returned by the `resolve_responsibility` call MUST reflect the
   ...
   entry) when REQ-014.1.1 applies.
