# REQ-048: Credential scoping model
...
### REQ-048.2: Task-capability derivation
...
### REQ-048.3: Runtime exposure and lifecycle
...
### REQ-048.4: Self-readable surface
...
2. A task capability used on a task collection MUST expose only its own task plus governed descendants authorized by REQ-048.7.
...
### REQ-048.9: Optional fleet-read configuration
...
2. Before endpoint or MCP argument validation, a configured fleet read token MUST avoid authentication-layer 401 or scope-layer 403 on every protected non-liveness `GET` or `HEAD` REST route except runner-owned `GET /tasks/{task_id}/session/input` polling, and receive authentication-layer 401 on that polling request, every mutation, `/tasks/{task_id}/live` or `/runners/{runner_id}/live` request, MCP tool call, and MCP resource read.

### REQ-048.10: Browser bearer transport
...
### REQ-048.11: Explicit-origin CORS
...
### REQ-048.12: Deterministic authorization boundary
