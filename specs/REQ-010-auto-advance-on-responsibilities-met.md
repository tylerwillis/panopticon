# REQ-010: Auto-advance on responsibilities met
...
4. `resolve_responsibility` MUST NOT transition a task out of a state that has no
   ...
   declared `advance`).
5. In the case of REQ-010.1.4, the task MUST remain in that state with every
   responsibility resolved.

### REQ-010.2: Parity with an explicit advance

1. A transition performed under REQ-010.1.1 MUST append the same history entry,
...
2. The task object returned by the `resolve_responsibility` call MUST reflect the
   ...
   entry) when REQ-010.1.1 applies.
