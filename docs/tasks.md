...
| `blocked` | A deliberate "waiting on something" marker the agent sets. A turn-to-agent write and every state change clear it; a turn-to-user write preserves it, and the agent may set it again if still stuck. |
